function [ numberOfCoefficients ] = cost_function_sorted( y , mask)
...
[x,~,exitflag] = linprog(zeros(l,1),[],[],H',zeros(sum(mask),1),ones(l,1),2^10*ones(l,1),[], options);
if size(x,1) ~= l
    x = ones(l,1);
end
a = Q' * x;
a(indices(logical(mask)),:) = 0 ;
res = Q * a;
